...
showNumberP1:
   ...
   mov rax, [rel number]
   xor rcx, rcx						; i = 0 (contador de 6 dígitos)
   cmp rax, 0
   jl sn_num_zero
   cmp rax, 999999
   jg sn_num_999999
   ...
   sn_num_999999:
		mov rax, 999999
   ; Recorremos de unidades a centenas de millar (6 posiciones).
   sn_for:
			cmp rcx, 6					; while (i) < 6
			jge sn_end
			mov byte [rel charac], 32	; por defecto mostramos espacio
			cmp rax, 0 					; al llegar a 0 mantenemos alineación derecha
			jle sn_for_zero

			xor rdx, rdx
			mov rbx, 10					; base decimal
			div rbx						; rax=cociente, rdx=residuo
			add dl, 48 					; convertir dígito [0..9] a ASCII
			mov byte [rel charac], dl
   sn_for_zero: 
		call gotoxyP1
		call printchP1
			dec dword [rel colScreen]	; siguiente dígito a la izquierda
			inc rcx
			jmp sn_for
   sn_end: 
		;restaurar el estado de los registros que se han guardado en la pila.
...
updateBoardP1:
   ...
   ub_for:
	...
	ub_inner_for:
				...
				imul eax, DIMMATRIX
				add eax, r15d
				cdqe								; ampliar index a 64 bits para direccionamiento

				mov eax, dword [m + rax * 4]
				mov qword [rel number], rax

				mov dword [rel rowScreen], r12d
				mov dword [rel colScreen], r14d
				call showNumberP1

				add r14d, 9							; siguiente celda visual en la misma fila
				inc r15d
				jmp ub_inner_for
   ub_for_inc:
		add r12d, 2
		...
		jmp ub_for
	ub_end:
			; Mostrar score en la zona inferior del tablero.
			mov rax, qword [rel score]
			mov qword [rel number], rax
			mov dword [rel rowScreen], 18
			mov dword [rel colScreen], 35
   		call showNumberP1

		mov dword [rel rowScreen], 18
		mov dword [rel colScreen], 36
   		call gotoxyP1
   ;restaurar el estado de los registros que se han guardado en la pila.
   pop r15
   pop r14
   ...
   rm_for:
		...
		jge rm_end
   		...
   		rm_inner_for:
			...
			cmp byte [rel dir], 76 ; 'L' ascii code
			jne rm_check_r

				; Rotación izquierda: destino [DIMMATRIX-1-j][i]
				mov ebx, DIMMATRIX
				dec ebx
				sub ebx, r13d
			...
			rm_check_r:
				cmp byte [rel dir], 82 ; 'R' ascii code
				jne rm_inner_for_inc
				
					; Rotación derecha: destino [j][DIMMATRIX-1-i]
					mov ebx, DIMMATRIX
					dec ebx
					sub ebx, r12d
...
shiftNumbersLP1:
   ...
   shn_for:
		...
		xor r13d, r13d
		shn_inner_for:
				; j < DIMMATRIX-1: la última columna no necesita búsqueda a la derecha.
				; Usamos eax para no modificar rbx (registro callee-saved).
				mov eax, DIMMATRIX-1
				cmp r13d, eax
				jge shn_for_inc
			...
			shn_after_while:
				...
				mov dword [m + rax * 4], 0
					...
					mov word [rel state], 2		; hubo movimiento en la fila
		shn_inner_for_inc:
				inc r13d							; saltar la celda ya fusionada (j+1)
			jmp shn_inner_for
   shn_for_inc:
   		inc r12d
		jmp shn_for
...
addPairsLP1:
   ...
   ap_after_for:
		...
		mov eax, r14d
		cdqe
		add qword [rel score], rax
		mov word [rel state], 2
   ap_end:                         
   ;restaurar el estado de los registros que se han guardado en la pila.
   pop r14 
   pop r13 
