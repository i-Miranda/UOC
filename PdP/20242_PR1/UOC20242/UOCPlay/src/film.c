...
tApiError catalog_add(tCatalog* catalog, tFilm film) {
	...
	// PR1_2b
	/////////////////////////////////
	tFilmListNode filmListNode;
	tFreeFilmListNode freeFilmListNode;
	tApiError apiError;

	assert(catalog != NULL && &film != NULL);	

	apiError = filmList_add(&catalog->filmList, film);
	if (apiError == E_FILM_DUPLICATED) {
		return apiError;
	}
	//If the film was added, and isFree add the address to the freeFilmList
	if (apiError == E_SUCCESS && film.isFree == true) {
		apiError = freeFilmList_add(&catalog->freeFilmList, &catalog->filmList.last->elem);
		if (apiError == E_FILM_DUPLICATED) {
			return apiError;
		}
	}
...
tApiError catalog_del(tCatalog* catalog, const char* name) {
	...
	// If it's on the freeFilmList, remove it
	freeFilmList_del(&catalog->freeFilmList, name);
	// If it's on the filmList, remove it
	apiError = filmList_del(&catalog->filmList, name);
	/////////////////////////////////
    return apiError;
}
